<head><meta charset="UTF-8"></head><h1 class="heading">InnoDB数据页结构</h1>
<p>标签： MySQL是怎样运行的</p>
<hr>
<h2 class="heading">不同类型的页简介</h2>
...
<h2 class="heading">数据页结构的快速浏览</h2>
<p>数据页代表的这块<code>16KB</code>大小的存储空间可以被划分为多个部分，不同部分有不同的功能，各个部分如图所示：</p>
<p></p><figure><img alt="image_1d5g808sn1bmg2861d5b1b32lht19.png-77.5kB" src="https://user-gold-cdn.xitu.io/2019/5/8/16a95c0ff3068b29?w=499&amp;h=551&amp;f=png&amp;s=79402"><figcaption></figcaption></figure><p></p>
...
</p></blockquote><h2 class="heading">记录在页中的存储</h2>
...
<p></p><figure><img alt="image_1cosvi1in9st476cdqfki1n39m.png-133.8kB" src="https://user-gold-cdn.xitu.io/2019/5/8/16a95c0fe86555ed?w=1272&amp;h=475&amp;f=png&amp;s=137021"><figcaption></figcaption></figure><p></p>
...
</code></pre><p>这个新创建的<code>page_demo</code>表有3个列，其中<code>c1</code>和<code>c2</code>列是用来存储整数的，<code>c3</code>列是用来存储字符串的。需要注意的是，<span style="color:red">我们把 <em><strong>c1</strong></em> 列指定为主键，所以在具体的行格式中InnoDB就没必要为我们去创建那个所谓的 <em><strong>row_id</strong></em> 隐藏列了</span>。而且我们为这个表指定了<code>ascii</code>字符集以及<code>Compact</code>的行格式。所以这个表中记录的行格式示意图就是这样的：</p>
<p></p><figure><img alt="image_1c9o2eib2vl11qnf1dfl1d2lco313.png-76.4kB" src="https://user-gold-cdn.xitu.io/2019/5/8/16a95c0feca77be3?w=1121&amp;h=474&amp;f=png&amp;s=78271"><figcaption></figcaption></figure><p></p>
...
<p></p><figure><img alt="image_1c9o52lt41v5c7vk1vm91fsm174b2d.png-49.5kB" src="https://user-gold-cdn.xitu.io/2019/5/8/16a95c0ff1ae5364?w=759&amp;h=347&amp;f=png&amp;s=50721"><figcaption></figcaption></figure><p></p>
...
<p></p><figure><img alt="image_1c9qs0j281knc16hc1hqsgj01v0o2c.png-82.8kB" src="https://user-gold-cdn.xitu.io/2019/5/8/16a95c0ff83f9870?w=948&amp;h=333&amp;f=png&amp;s=84803"><figcaption></figcaption></figure><p></p>
...
<p>是的，记录也可以比大小，对于<span style="color:red">一条完整的记录</span>来说，比较记录的大小就是比较<code>主键</code>的大小。比方说我们插入的4行记录的主键值分别是：<code>1</code>、<code>2</code>、<code>3</code>、<code>4</code>，这也就意味着这4条记录的大小从小到大依次递增。</p>
<blockquote class="warning"><p>小贴士：

请注意我强调了对于`一条完整的记录`来说，比较记录的大小就相当于比的是主键的大小。后边我们还会介绍只存储一条记录的部分列的情况，敬请期待～
</p></blockquote><p>但是不管我们向<code>页</code>中插入了多少自己的记录，设计<code>InnoDB</code>的大叔们都规定他们定义的两条伪记录分别为最小记录与最大记录。这两条记录的构造十分简单，都是由5字节大小的<code>记录头信息</code>和8字节大小的一个固定的部分组成的，如图所示</p>
<p></p><figure><img alt="image_1c9ra45eam7t1mil9o1h3ucqdhv.png-50.4kB" src="https://user-gold-cdn.xitu.io/2019/5/8/16a95c100ff0ccc2?w=576&amp;h=395&amp;f=png&amp;s=51648"><figcaption></figcaption></figure><p></p>
<p>由于这两条记录不是我们自己定义的记录，所以它们并不存放在<code>页</code>的<code>User Records</code>部分，他们被单独放在一个称为<code>Infimum + Supremum</code>的部分，如图所示：</p>
<p></p><figure><img alt="image_1c9qs1mn2t3j1nt344116nk15uf2p.png-119.7kB" src="https://user-gold-cdn.xitu.io/2019/5/8/16a95c10773d8cee"><figcaption></figcaption></figure><p></p>
...
<p>这玩意儿非常重要，它表示<span style="color:red">从当前记录的真实数据到下一条记录的真实数据的地址偏移量</span>。比方说第一条记录的<code>next_record</code>值为<code>32</code>，意味着从第一条记录的真实数据的地址处向后找<code>32</code>个字节便是下一条记录的真实数据。如果你熟悉数据结构的话，就立即明白了，这其实是个<code>链表</code>，可以通过一条记录找到它的下一条记录。但是需要注意注意再注意的一点是，<span style="color:red"><code>下一条记录</code>指得并不是按照我们插入顺序的下一条记录，而是按照主键值由小到大的顺序的下一条记录</span>。而且规定<span style="color:red"> <em><strong>Infimum记录（也就是最小记录）</strong></em> 的下一条记录就是本页中主键值最小的用户记录，而本页中主键值最大的用户记录的下一条记录就是 <em><strong>Supremum记录（也就是最大记录）</strong></em>  </span>，为了更形象的表示一下这个<code>next_record</code>起到的作用，我们用箭头来替代一下<code>next_record</code>中的地址偏移量：</p>
<p></p><figure><img alt="image_1cot1r96210ph1jng1td41ouj85c13.png-120.5kB" src="https://user-gold-cdn.xitu.io/2019/5/8/16a95c1084c440b4?w=1044&amp;h=549&amp;f=png&amp;s=123443"><figcaption></figcaption></figure><p></p>
<p>从图中可以看出来，<span style="color:red">我们的记录按照主键从小到大的顺序形成了一个单链表</span>。<code>最大记录</code>的<code>next_record</code>的值为<code>0</code>，这也就是说最大记录是没有<code>下一条记录</code>了，它是这个单链表中的最后一个节点。如果从中删除掉一条记录，这个链表也是会跟着变化的，比如我们把第2条记录删掉：</p>
<pre><code class="hljs bash" lang="bash">mysql&gt; DELETE FROM page_demo WHERE c1 = 2;
Query OK, 1 row affected (0.02 sec)
</code></pre><p>删掉第2条记录后的示意图就是：</p>
<p></p><figure><img alt="image_1cul8slbp1om0p31b3u1be11gco9.png-119.6kB" src="https://user-gold-cdn.xitu.io/2019/5/8/16a95c108ee1da43?w=1032&amp;h=528&amp;f=png&amp;s=122514"><figcaption></figcaption></figure><p></p>
...
<p></p><figure><img alt="image_1cot2j9n94a511jd15clrrfp6p1t.png-137.8kB" src="https://user-gold-cdn.xitu.io/2019/5/8/16a95c109f25d55d?w=1120&amp;h=557&amp;f=png&amp;s=141111"><figcaption></figcaption></figure><p></p>
<p>从图中可以看到，<code>InnoDB</code>并没有因为新记录的插入而为它申请新的存储空间，而是直接复用了原来被删除记录的存储空间。</p>
<blockquote class="warning"><p>小贴士：

当数据页中存在多条被删除掉的记录时，这些记录的next_record属性将会把这些被删除掉的记录组成一个垃圾链表，以备之后重用这部分存储空间。
</p></blockquote><h2 class="heading">Page Directory（页目录）</h2>
...
</code></pre><p>最笨的办法：从<code>Infimum</code>记录（最小记录）开始，沿着链表一直往后找，总有一天会找到（或者找不到[摊手]），在找的时候还能投机取巧，因为链表中各个记录的值是按照从小到大顺序排列的，所以当链表的某个节点代表的记录的主键值大于你想要查找的主键值时，你就可以停止查找了，因为该节点后边的节点的主键值依次递增。</p>
...
<li>
<p>每个组的<span style="color:red">最后</span>一条记录（也就是组内最大的那条记录）的头信息中的<code>n_owned</code>属性表示该记录拥有多少条记录，也就是该组内共有几条记录。</p>
</li>
<li>
...
<p></p><figure><img alt="image_1couapvdmb5mvm1i0l5m0vcb2a.png-128.2kB" src="https://user-gold-cdn.xitu.io/2019/5/8/16a95c10c57164a6?w=1011&amp;h=540&amp;f=png&amp;s=131292"><figcaption></figcaption></figure><p></p>
<p>从这个图中我们需要注意这么几点：</p>
<ul>
<li>
...
<p></p><figure><img alt="image_1couarh4no6j1a8q9htilr13qc2n.png-105.1kB" src="https://user-gold-cdn.xitu.io/2019/5/8/16a95c10dac72472?w=1032&amp;h=537&amp;f=png&amp;s=107616"><figcaption></figcaption></figure><p></p>
<p>哎呀，咋看上去怪怪的，这么乱的图对于我这个强迫症真是不能忍，那我们就暂时不管各条记录在存储设备上的排列方式了，单纯从<span style="color:red">逻辑上</span>看一下这些记录和页目录的关系：</p>
<p></p><figure><img alt="image_1couate3jr19gc18gl1cva1fcg34.png-100.8kB" src="https://user-gold-cdn.xitu.io/2019/5/8/16a95c10f2e61ad5?w=994&amp;h=550&amp;f=png&amp;s=103256"><figcaption></figcaption></figure><p></p>
<p>这样看就顺眼多了嘛！为什么最小记录的<code>n_owned</code>值为1，而最大记录的<code>n_owned</code>值为<code>5</code>呢，这里头有什么猫腻么？</p>
<p>是的，设计<code>InnoDB</code>的大叔们对每个分组中的记录条数是有规定的：<span style="color:red">对于最小记录所在的分组只能有 <em><strong>1</strong></em> 条记录，最大记录所在的分组拥有的记录条数只能在 <em><strong>1~8</strong></em> 条之间，剩下的分组中记录的条数范围只能在是 <em><strong>4~8</strong></em> 条之间</span>。所以分组是按照下边的步骤进行的：</p>
<ul>
<li>
...
<pre><code class="hljs bash" lang="bash">mysql&gt; INSERT INTO page_demo VALUES(5, 500, <span class="hljs-string">'eeee'</span>), (6, 600, <span class="hljs-string">'ffff'</span>), (7, 700, <span class="hljs-string">'gggg'</span>), (8, 800, <span class="hljs-string">'hhhh'</span>), (9, 900, <span class="hljs-string">'iiii'</span>), (10, 1000, <span class="hljs-string">'jjjj'</span>), (11, 1100, <span class="hljs-string">'kkkk'</span>), (12, 1200, <span class="hljs-string">'llll'</span>), (13, 1300, <span class="hljs-string">'mmmm'</span>), (14, 1400, <span class="hljs-string">'nnnn'</span>), (15, 1500, <span class="hljs-string">'oooo'</span>), (16, 1600, <span class="hljs-string">'pppp'</span>);
Query OK, 12 rows affected (0.00 sec)
Records: 12  Duplicates: 0  Warnings: 0
</code></pre><p>哈，我们一口气又往表中添加了12条记录，现在页里边就一共有18条记录了（包括最小和最大记录），这些记录被分成了5个组，如图所示：</p>
<p></p><figure><img alt="image_1d6g64af2sgj1816ktl1q22dehp.png-189.1kB" src="https://user-gold-cdn.xitu.io/2019/5/8/16a95c10e3449897?w=1034&amp;h=638&amp;f=png&amp;s=193593"><figcaption></figcaption></figure><p></p>
...
<ol>
<li>
<p><span style="color:red">通过二分法确定该记录所在的槽，并找到该槽中主键值最小的那条记录</span>。</p>
</li>
<li>
<p><span style="color:red">通过记录的<code>next_record</code>属性遍历该槽所在的组中的各个记录</span>。</p>
</li>
</ol>
<blockquote class="warning"><p>小贴士：

如果你不知道二分法是个什么东西，找个基础算法书看看吧。什么？算法书写的看不懂？等我～
</p></blockquote><h2 class="heading">Page Header（页面头部）</h2>
...
</p></blockquote><h2 class="heading">File Header（文件头部）</h2>
...
<p>我们前边强调过，<code>InnoDB</code>都是以页为单位存放数据的，有时候我们存放某种类型的数据占用的空间非常大（比方说一张表中可以有成千上万条记录），<code>InnoDB</code>可能不可以一次性为这么多数据分配一个非常大的存储空间，如果分散到多个不连续的页中存储的话需要把这些页关联起来，<code>FIL_PAGE_PREV</code>和<code>FIL_PAGE_NEXT</code>就分别代表本页的上一个和下一个页的页号。这样通过建立一个双向链表把许许多多的页就都串联起来了，而无需这些页在物理上真正连着。需要注意的是，<span style="color:red">并不是所有类型的页都有上一个和下一个页的属性</span>，不过我们本集中唠叨的<code>数据页</code>（也就是类型为<code>FIL_PAGE_INDEX</code>的页）是有这两个属性的，所以所有的数据页其实是一个双链表，就像这样：</p>
<p></p><figure><img alt="image_1ca00fhg418pl1f1a1iav1uo3aou9.png-90.9kB" src="https://user-gold-cdn.xitu.io/2019/5/8/16a95c10eb9d61ce?w=898&amp;h=365&amp;f=png&amp;s=93102"><figcaption></figcaption></figure><p></p>
</li>
</ul>
<p>关于<code>File Header</code>的其他属性我们暂时用不到，等用到的时候再提哈～</p>
<h2 class="heading">File Trailer</h2>
<p>我们知道<code>InnoDB</code>存储引擎会把数据存储到磁盘上，但是磁盘速度太慢，需要以<code>页</code>为单位把数据加载到内存中处理，如果该页中的数据在内存中被修改了，那么在修改后的某个时间需要把数据<span style="color:red">同步</span>到磁盘中。但是在同步了一半的时候中断电了咋办，这不是莫名尴尬么？为了检测一个页是否完整（也就是在同步的时候有没有发生只同步一半的尴尬情况），设计<code>InnoDB</code>的大叔们在每个页的尾部都加了一个<code>File Trailer</code>部分，这个部分由<code>8</code>个字节组成，可以分成2个小部分：</p>
<ul>
<li>
<p>前4个字节代表页的校验和</p>
...
<li>
<p><code>InnoDB</code>会为把页中的记录划分为若干个组，每个组的最后一个记录的地址偏移量作为一个<code>槽</code>，存放在<code>Page Directory</code>中，所以在一个页中根据主键查找记录是非常快的，分为两步：</p>
<ul>
<li>
<p>通过二分法确定该记录所在的槽。</p>
</li>
