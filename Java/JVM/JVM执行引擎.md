...
- 那么，如果想让一个Java程序运行起来、
  ...
  ![img](https://user-gold-cdn.xitu.io/2020/7/25/173866f70a49053e?imageView2/0/w/1280/h/960/format/webp/ignore-error/1)
...
- 以及通过对象头（堆）中的元数据指针，定位到目标对象的类型信息（方法区）。（下图堆区与方法区之间，其实还存在类型指针：从堆区对象实例指向方法区的类元数据InstanceKlass）
  ...
  ![img](https://user-gold-cdn.xitu.io/2020/7/25/1738671d4d825655?imageView2/0/w/1280/h/960/format/webp/ignore-error/1)

### 三、Java代码编译和执行过程

大部分的程序代码转换成物理机的目标代码或虚拟机能执行的指令集之前，都需要经过下面图中的各个步骤：

![img](https://user-gold-cdn.xitu.io/2020/7/25/173867508b9db3a4?imageView2/0/w/1280/h/960/format/webp/ignore-error/1)

Java代码编译是由Java源码编译器（Java前端编译器）来完成(与JVM无关)，流程图如下所示：

![img](https://user-gold-cdn.xitu.io/2020/7/25/1738675ae1fb5a70?imageView2/0/w/1280/h/960/format/webp/ignore-error/1)

Java字节码的执行是由JVM执行引擎来完成，流程图如下所示：

![img](https://user-gold-cdn.xitu.io/2020/7/25/17386763bde4e856?imageView2/0/w/1280/h/960/format/webp/ignore-error/1)



### 四、为什么说Java是半解释半编译型语言？
...
#### 1.基于目前JVM执行引擎当中解释器与即时编译器共存

现在JVM在执行Java代码的时候，会将解释执行与编译执行二者结合起来进行。如今Java采用的是解释和编译混合的模式：

![img](https://user-gold-cdn.xitu.io/2020/7/25/173867766bab0551?imageView2/0/w/1280/h/960/format/webp/ignore-error/1)
...
![img](https://user-gold-cdn.xitu.io/2020/7/25/1738678646d3c78b?imageView2/0/w/1280/h/960/format/webp/ignore-error/1)
...
- 当一个方法被调用时， 会先检查该方法是否存在被JIT编译过的版本，如 果存在，则优先使用编译后的本地代码来执行。如果不存在已被编译过的版本，则将此方法的调用计数器值加1，然后判断方法调用计数器与回边计数器值之和是否超过方法调用计数器的阈值。如果已超过阈值，那么将会向即时编译器提交一个该方法的代码编译请求。
  ...
  ![img](https://user-gold-cdn.xitu.io/2020/7/25/173867fc5e4f353c?imageView2/0/w/1280/h/960/format/webp/ignore-error/1)
...
![img](data:image/svg+xml;utf8,<?xml version="1.0"?><svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="967" height="918"></svg>)
...
![img](data:image/svg+xml;utf8,<?xml version="1.0"?><svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="1280" height="784"></svg>)
